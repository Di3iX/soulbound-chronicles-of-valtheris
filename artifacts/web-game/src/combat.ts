...
export const makeLocationEnemies = (loc: LocationId): Enemy[] => {
  ...
  const spawns: Record<LocationId, Spawn[]> = {
    ...
    mountains: [
      ...
      { name: 'Снежный паук', x: 13, y: 16 }, { name: 'Йети', x: 16, y: 19 },
    ...
    mine: [
      ...
      { name: 'Голем', x: 13, y: 14 }, { name: 'Летучая мышь', x: 20, y: 19 },
      { name: 'Шахтёр-зомби', x: 10, y: 19 },
    ],
    pass: [
      { name: 'Горный тролль', x: 14, y: 10 }, { name: 'Гарпия', x: 15, y: 13 },
      { name: 'Голем', x: 13, y: 17 }, { name: 'Гарпия', x: 14, y: 20 },
      { name: 'Горный тролль', x: 15, y: 22 },
    ],
    icefort: [
